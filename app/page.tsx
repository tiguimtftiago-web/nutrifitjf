    ...
    <main className="min-h-screen bg-[#080a07] text-white">
      ...
      <section id="sucos" className="mx-auto max-w-7xl px-5 py-20 md:px-8">
        <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-[#171d10] to-[#0e110c] p-8 md:p-12">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 md:grid-cols-3">{functionalJuices.map(([name,image]) => <a href={whatsapp} key={name} className="group overflow-hidden rounded-2xl border border-white/10 bg-black/20 hover:border-[#a7b86a]/40"><div className="aspect-[4/3] overflow-hidden"><img src={image} alt={name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" /></div><div className="p-4"><div className="font-black">{name}</div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><span className="rounded-lg bg-white/5 px-2 py-2 text-white/60">500 ml <b className="block text-sm text-[#ef7d18]">R$ 12,90</b></span><span className="rounded-lg bg-white/5 px-2 py-2 text-white/60">300 ml <b className="block text-sm text-[#ef7d18]">R$ 9,90</b></span></div></div></a>)}</div>
          <div className="mt-10 border-t border-white/10 pt-8"><div className="text-sm font-black uppercase tracking-wider text-[#ef7d18]">Sucos Nutrifit • 500 ml R$ 12,00 • 300 ml R$ 9,90</div><div className="mt-4 grid gap-4 sm:grid-cols-3">{naturalJuices.map(([name,image]) => <a href={whatsapp} key={name} className="group overflow-hidden rounded-2xl border border-white/10 bg-black/20 hover:border-[#a7b86a]/40"><div className="aspect-[4/3] overflow-hidden"><img src={image} alt={name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" /></div><div className="p-4"><div className="font-black">{name}</div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><span className="rounded-lg bg-white/5 px-2 py-2 text-white/60">500 ml <b className="block text-sm text-[#ef7d18]">R$ 12,00</b></span><span className="rounded-lg bg-white/5 px-2 py-2 text-white/60">300 ml <b className="block text-sm text-[#ef7d18]">R$ 9,90</b></span></div></div></a>)}</div></div>
        </div>
      </section>
