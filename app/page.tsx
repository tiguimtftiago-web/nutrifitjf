...
const products = [
  ["Patinho com Purê de Abóbora","Linha Fit • 350 g","R$ 19,90","Patinho moído bem temperado com purê cremoso de abóbora.","/images/hero.jpg"],
  ["Patinho com Batata-Doce","Linha Fit • 350 g","R$ 19,90","Patinho moído temperado com purê cremoso de batata-doce.","/images/hero.jpg"],
  ["Frango Grelhado com Mix de Legumes","Linha Fit • 350 g","R$ 19,90","Peito de frango grelhado com mix de legumes selecionados."],
  ["Carne Acebolada com Legumes","Linha Fit • 350 g","R$ 19,90","Carne bovina acebolada acompanhada de legumes variados."],
  ["Lombo Suíno com Legumes Assados","Linha Fit • 350 g","R$ 19,90","Lombo suíno temperado com legumes assados."],
  ["Frango em Cubos ao Molho de Ervas","Linha Fit • 350 g","R$ 19,90","Frango em cubos ao molho de ervas com legumes."],
  ["Patinho com Legumes ao Toque Amanteigado","Linha Fit • 350 g","R$ 19,90","Patinho moído acompanhado de legumes ao toque amanteigado."],
  ["Strogonoff Leve de Frango","Linha 500 g","R$ 26,90","Frango em molho cremoso de strogonoff, arroz e batata palha."],
  ["Frango Cremoso Fit","Linha 500 g","R$ 27,90","Frango ao molho cremoso leve, arroz e legumes."],
  ["Churrasco Fit Mineiro","Linha 500 g","R$ 29,90","Contra-filé grelhado com arroz, vinagrete e farofa caseira."],
  ["Parmegiana Cremosa","Linha 500 g","R$ 29,90","Filé de frango empanado, molho de tomate, queijo e acompanhamento."],
  ["Executive Fit — Lombo Suíno","Linha 500 g","R$ 28,90","Lombo suíno grelhado com macarrão alho e óleo e legumes."],
  ["Feijoada Fit","Linha 500 g","R$ 28,90","Feijoada leve com lombo e calabresa, arroz, couve e farofa."],
  ["Escondidinho Fit","Linha 500 g","R$ 27,90","Purê cremoso com carne temperada e finalização gratinada."],
  ["Beef & Sweet Potato","Linha Performance • 350 g","R$ 24,90","Carne bovina, batata-doce, ovo e brócolis."],
  ["Chicken Rice Performance","Linha Performance • 350 g","R$ 22,90","Frango grelhado, arroz integral, ovo e legumes."],
  ["Patinho Strong","Linha Performance • 350 g","R$ 24,90","Patinho, batata-doce, ovos cozidos e brócolis."],
  ["Chicken Power","Linha Performance • 350 g","R$ 22,90","Frango grelhado em cubos, arroz, ovo e legumes."],
  ["Chicken Sweet Potato","Linha Performance • 350 g","R$ 22,90","Frango grelhado, batata-doce, brócolis e cenoura."],
...
const combos350 = [["5 marmitas","R$ 94,90"],["7 marmitas","R$ 129,90"],["10 marmitas","R$ 184,90"],["14 marmitas","R$ 249,90"],["20 marmitas","R$ 349,90"]];
const combos500 = [["5 marmitas","R$ 109,90"],["7 marmitas","R$ 149,90"],["10 marmitas","R$ 209,90"]];
    ...
    <main className="min-h-screen bg-[#090b08] text-white">
      ...
      <section id="inicio" className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-12 md:grid-cols-[1fr_1fr] md:px-8 md:py-20">
          ...
          <div className="relative z-10 overflow-hidden rounded-[2rem] border border-white/10 bg-black/30 shadow-2xl shadow-black/30"><img src="/images/hero.jpg" alt="Marmita Nutrifit" className="h-full min-h-[360px] w-full object-cover"/><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent p-7 pt-28"><div className="text-xs font-bold uppercase tracking-[.18em] text-[#a7b86a]">Destaque da casa</div><div className="mt-1 text-2xl font-black">Patinho com purê de abóbora</div><div className="mt-1 text-[#ef7d18] font-bold">R$ 19,90 • 350 g</div></div></div>
        </div>
      </section>
      ...
      <section id="combos" className="mx-auto max-w-7xl px-5 py-20 md:px-8"><div className="text-sm font-bold uppercase tracking-[.2em] text-[#ef7d18]">Combos</div><h2 className="mt-2 text-4xl font-black md:text-5xl">Monte sua semana e economize</h2><div className="mt-10 grid gap-10 lg:grid-cols-2"><div><div className="mb-4 text-lg font-bold">Combos 350 g</div><div className="grid gap-3 sm:grid-cols-2">{combos350.map(([label,price]) => <a key={label} href={whatsapp} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[.035] p-5"><span className="font-semibold">{label}</span><span className="font-black text-[#ef7d18]">{price}</span></a>)}</div></div><div><div className="mb-4 text-lg font-bold">Combos 500 g</div><div className="grid gap-3">{combos500.map(([label,price]) => <a key={label} href={whatsapp} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[.035] p-5"><span className="font-semibold">{label}</span><span className="font-black text-[#ef7d18]">{price}</span></a>)}</div></div></div></section>

      <section id="como-pedir" className="border-t border-white/10 bg-[#0f110d]"><div className="mx-auto max-w-7xl px-5 py-20 md:px-8"><div className="grid gap-5 md:grid-cols-3">{[["01","Escolha","Selecione seus sabores e quantidades."],["02","Monte o pedido","Clique no WhatsApp e envie sua lista."],["03","Receba","Combine entrega e pagamento direto com a Nutrifit."]].map(([n,t,d]) => <div key={n} className="rounded-3xl border border-white/10 bg-white/[.03] p-7"><div className="text-sm font-black text-[#ef7d18]">{n}</div><h3 className="mt-3 text-2xl font-black">{t}</h3><p className="mt-3 leading-7 text-white/55">{d}</p></div>)}</div><div className="mt-10 flex flex-col items-start justify-between gap-7 rounded-[2rem] border border-[#a7b86a]/20 bg-[#171c11] p-8 md:flex-row md:items-center"><div><div className="text-sm font-bold uppercase tracking-[.2em] text-[#a7b86a]">Pedidos</div><h3 className="mt-2 text-3xl font-black">Fale diretamente com a Nutrifit</h3><p className="mt-2 text-white/55">WhatsApp (32) 99803-0038 • Instagram @nutrifit_jf</p></div><a href={whatsapp} className="inline-flex items-center gap-2 rounded-full bg-[#ef7d18] px-7 py-4 font-black text-black"><ShoppingBag size={19}/> Fazer pedido</a></div></div></section>

      <footer className="border-t border-white/10"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-8 text-sm text-white/45 md:flex-row md:items-center md:justify-between md:px-8"><div className="text-lg font-black text-white">nutri<span className="text-[#ef7d18]">fit</span></div><div>Juiz de Fora – MG • @nutrifit_jf • (32) 99803-0038</div></div></footer>
    </main>
  );
}
